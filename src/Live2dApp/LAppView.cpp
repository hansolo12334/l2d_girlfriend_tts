...
//hansolo
void  LAppView::scale_user(double x, double y, double scale)
{
    Csm::csmFloat32 cx = static_cast<Csm::csmFloat32>(x);
    Csm::csmFloat32 cy = static_cast<Csm::csmFloat32>(y);

    Csm::csmFloat32 csm_scale = static_cast<Csm::csmFloat32>(scale);

    // LAppPal::PrintLog("[APP]adjustScale_user cx:%.2f cy:%.2f scale:%.2f", cx, cy,csm_scale);
    // _viewMatrix->AdjustScale(cx, cy, csm_scale);
    _viewMatrix->Scale(csm_scale, csm_scale);
}

void  LAppView::translate_user(float x, float y)
{
    Csm::csmFloat32 screenX = _deviceToScreen->TransformX(x);
    Csm::csmFloat32 screenY = _deviceToScreen->TransformY(y);
    // LAppPal::PrintLog("[APP]adjustTranslate_user screenX:%.2f screenY:%.2f", screenX, screenY);
    _viewMatrix->Translate(screenX, screenY);
}
